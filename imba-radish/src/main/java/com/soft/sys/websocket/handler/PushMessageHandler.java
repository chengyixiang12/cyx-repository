...
import com.soft.sys.websocket.receive.PushMessageRequest;
import com.soft.sys.websocket.send.PushMessageResponse;
...
public class PushMessageHandler implements WebSocketConcreteHandler<String> {
    ...
    public void handle(WebSocketSession session, AbstractWebSocketMessage<String> message) throws IOException {
        PushMessageRequest pushMessageRecParam = JSON.parseObject(message.getPayload(), PushMessageRequest.class);
        JSONArray receivers = pushMessageRecParam.getReceivers();
        PushMessageResponse pushMessageSendParams = new PushMessageResponse();
